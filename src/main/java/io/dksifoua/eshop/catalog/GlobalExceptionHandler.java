...
public class GlobalExceptionHandler implements WebExceptionHandler {
    ...
    public @Nonnull Mono<Void> handle(@Nonnull ServerWebExchange exchange, Throwable ex) {
        return switch (ex.getCause()) {
            case MongoWriteException e -> handleDuplicateKeyException(exchange, e);
            default -> throw new IllegalStateException("Unexpected value: " + ex.getLocalizedMessage());
        };
    }
}
